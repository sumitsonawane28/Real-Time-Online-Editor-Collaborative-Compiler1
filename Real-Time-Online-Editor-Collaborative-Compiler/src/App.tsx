...
export default function App() {
  ...
  const handleNewFile = async (name: string, language: string) => {
    ...
    setFiles((prev) => [...prev, { id, name, language, content: '', isOpen: true }]);
    setActiveFileId(id);
    // Persist immediately with empty content so the file exists in DB
    if (user?.token) {
      try {
        const saved = await api.saveFile(name, '', user.token);
        // Replace temp id with real MongoDB _id
        setFiles((prev) => prev.map((f) => f.id === id ? { ...f, id: saved._id } : f));
